...
import com.eaglec.plat.utils.dbutils.HumpBeanProcessor;
import com.eaglec.plat.utils.dbutils.HumpMatcher;
...
public class WindowDao {
	...
	public void setDataSourceSpied(DataSource dataSourceSpied) {
		this.dataSourceSpied = dataSourceSpied;
	}
	 ...
	 * @return
	 */
	public Map<String,Map<String, Object>> findToMap(String sql,String mainKey) {
		queryRunner = new QueryRunner(dataSourceSpied);
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		...
		try {			
			...
			for(int i=0 ; i<list.size() ;i++){
				if(list.get(i).get(mainKey) == null){
					continue;
				}
				map.put(list.get(i).get(mainKey).toString(), list.get(i));
			}			
		} catch (SQLException e) {
			e.printStackTrace();
		}
	 ...
	 * @return
	 */
	public Map<String, Object> findTowFieldToMap(String sql, String keyName, String valueName) {
		queryRunner = new QueryRunner(dataSourceSpied);
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		...
		try {			
			list =   queryRunner.query(sql, new MapListHandler());
			for(int i=0 ; i<list.size() ;i++){
				if(list.get(i).get(keyName) == null){
					continue;
				}
				map.put(list.get(i).get(keyName).toString(), list.get(i).get(valueName));
			}			
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public List<Object[]> findToList(String sql) {
		queryRunner = new QueryRunner(dataSourceSpied);

 ...
 * @since 2013-12-05
 */
public class SellerGoodsOrderSyncBean extends AbstractSyncBean<GoodsOrder> {

	public SellerGoodsOrderSyncBean(GoodsOrder syncObj, SyncType syncType) {
		super(syncObj, syncType);
	}

	@Override
	protected void buildingBean(GoodsOrder syncObj) throws Exception {
		if (syncObj.getService().getEnterprise().getIndustryType() == null) {
			throw new Exception("enterprise.getIndustryType() is null");
		}
		wid = syncObj.getService().getEnterprise().getIndustryType();

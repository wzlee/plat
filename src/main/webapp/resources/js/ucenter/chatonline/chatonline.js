    ...
    tabpanel.getChatUsers = function(){
    	$('#chatusers').datagrid({
	        ...
	        columns:[[ 
	            ...
	            {field:'staffname',title:'昵称',align:'center',width:170},
	            {field:'email',title:'邮箱',align:'center', hidden:true}
	        ]],
	        onDblClickRow: function(rowIndex, rowData){
	        	var trows = $('#targetUsers').datagrid('getData').rows;
	        	if(!trows){
	        		var data = [];
	        		data.push(rowData);
	        		$('#targetUsers').datagrid('loadData', data);
	        	} else {
	        		$('#targetUsers').datagrid('appendRow', rowData);
	        	}
	        	$('#chatusers').datagrid('deleteRow', rowIndex);
	        	if(rowData.role){
	        		var deludata = $('#targetUsers').data("deludata");
	        		var str = '{\"username\":\"'+ rowData.username +'\",\"staffname\":\"'+ rowData.staffname+ '\"}';
	        		if(deludata){
	        			deludata = deludata.replace(str, '');
	        			$('#targetUsers').data("deludata", deludata);
	        		}
	        	}
	        }
	    });
	    
    	$('#targetUsers').datagrid({
    		title: '企业客服帐号',
	    	method:'post',
	        ...
	        onDblClickRow: function(rowIndex, rowData){
	        	var trows = $('#chatusers').datagrid('getData').rows;
	        	if(!trows){
	        		var data = [];
	        		data.push(rowData);
	        		$('#chatusers').datagrid('loadData', data);
	        	} else {
	        		$('#chatusers').datagrid('appendRow', rowData);
	        	}
	        	$('#targetUsers').datagrid('deleteRow', rowIndex);
	        	if(rowData.role){
	        		var deludata = $('#targetUsers').data("deludata");
	        		var str = '{\"username\":\"'+ rowData.username +'\",\"staffname\":\"'+ rowData.staffname+ '\"}';
	        		deludata ? (deludata += ','+ str) : (deludata = str);
	        		$('#targetUsers').data("deludata", deludata);
	        	}
	        }
	    });
	    $('#chatUserSave').bind('click', function(){
    		var userdata = '';
	    	var trows = $('#targetUsers').datagrid('getData').rows;
	    	if(trows){
	    		for(var i = 0; i < trows.length; i++){
	    			...
	    			if(!row.role){
		    			...
		    			userdata += '\",\"staffname\":\"'+row.staffname;
//	    				userdata += '\",\"role\":\"'+row.role;
	    				userdata += '\",\"email\":\"'+ (row.email ? row.email : null) +'\"}';
	    			}
	    		}
	    	}
	    	var deludata = $('#targetUsers').data("deludata");
	    	if(deludata){
	    		var tempstr = deludata.substring(0, 1);
	    		if(tempstr == ','){
	    			deludata = deludata.substring(1);
	    		}
	    		tempstr = deludata.substring(deludata.length - 1);
	    		if(tempstr == ','){
	    			deludata = deludata.substring(0, deludata.length - 1);
	    		}
	    	}
	    	$.ajax({
			   	type: 'post',
			   	url: 'chat/batchSaveUser',
			   	data: {
			   		userdata: userdata,
			   		deludata: deludata ? deludata : '',
			   		eid: tabpanel.attr('eid')
			   	},
			   	success: function(data){
			   		data = eval('('+ data +')');
	    			if(data.success){
	    				$('#targetUsers').data("uid", '');
	    				$('#targetUsers').datagrid('load');
	    				art.dialog({
							title: '错误',
							content: '客服用户保存成功！',
							ok: function () {
	        ...
	        toolbar: [{
				...
				handler: function(){
					...
					if (row) {
						art.dialog({
							title: '提示',
							content: '您确定要删除该客服吗？如果该机器人关联了客服组，那么该组将自动关联默认机器人！',
							ok: function () {
								$.ajax({
								   	type: 'post',

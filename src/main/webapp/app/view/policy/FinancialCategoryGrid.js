Ext.define('plat.view.policy.FinancialCategoryGrid', {
            extend : 'Ext.tree.Panel',
            xtype : 'financialcategorygrid',
            title : '专项资金类别管理',
            id : 'zxzjlbgl',
            closeAction : 'hide',
            rootVisible : false,
...
//                },
                ...
                {header : '添加时间', dataIndex : 'time', width : 140, align : 'center'},
                {
                        ...
                        isDisabled:function(view, rowIdx, colIdx, item, record){
                            if((record.data.type == 1) && (record.data.leaf == true)){
                                return true;
                            }
                         },
                        handler : function(grid, rowIndex, colIndex,
                            ...
                            //弹出窗口，加载pid,添加子类别
                            var _window = Ext.ComponentQuery.query('financialcategorywindow')[0];
                            if (!_window) {
                                _window = Ext.create('plat.view.policy.FinancialCategoryWin');
                            }
                            _window.show();
                            _window.setTitle('添加类别');
                            var _form = _window.down('financialcategoryform');

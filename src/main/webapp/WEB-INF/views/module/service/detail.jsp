...
<%@page import="com.eaglec.plat.domain.service.Service"%>
...
<meta charset="UTF-8">
<link type="text/css" rel="stylesheet" href="resources/css/main/detail.css?v=20131203am" />
<link type="text/css" rel="stylesheet" href="resources/css/main/company.css?v=20131203am" />
</head>
<body>
    ...
    <div class="column-l">
      <div class="thumb">
      	<c:if test="${service.picture.contains('http')}">
      		<img class="detailimg" src="${service.picture}" width="232" height="190" title="${service.serviceName }" onerror="nofind(this);" />
      	</c:if>
     	 <c:if test="${!service.picture.contains('http')}">
      		<img class="detailimg" src="upload/${service.picture}" width="232" height="190" title="${service.serviceName }" onerror="nofind(this);" />
      	</c:if>
      </div>
      <ul class="choose-btn">
		    <c:choose>
	      		<c:when test="${collected == true}">
	      			<li class="s1"><a class="active" href="javascript:void(0);" onclick="collect(this, ${service.id})">已收藏</a></li>
	      		</c:when>
	      		<c:otherwise>
					...
					<div class="top">
						...
						<div class="row">
			            	...
			            	<div class="online-consultation">
			            		<c:choose>
							   		<c:when test="${isRelate}">
					            		<a href="javascript:void(0);" onclick="i_im_talkAnontmity(${chatlineuser});">在线咨询</a>
							   		</c:when>
							   		<c:otherwise>
							   			<a href="javascript:void(0);" onclick="i_im_talkWin(${chatlineuser},'imWin');">在线咨询</a>
							   		</c:otherwise>
								</c:choose>
			            	</div>
	        			</div>
    ...
    <div class="column-r">
  	...
  	<div class="s-action">
  		<c:if test="${service.currentStatus == 3 }">
	      	<a class="apply-btn" id="apply-btn" href="javascript:void(0);" data-sid=${service.id } data-sname=${service.serviceName } data-sprice=${service.costPrice == null ? '面议':service.costPrice  }
				data-tel=${loginEnterprise.tel == null ? '暂无':loginEnterprise.tel } 
				data-email=${loginEnterprise.email == null ? '暂无': loginEnterprise.email}>申请服务
			</a>
		</c:if>
...
<script src="http://119.147.47.115/webim/runIndex.do?uid=${chatuserid}"></script>
</body>
</html>

...
<div class="wrap">
	<div class="crumb-nav">首页 &gt;
	<c:choose>
		<c:when test="${service.enterprise.type eq '3' }">服务机构</c:when>
		<c:when test="${service.enterprise.type eq '4' }">政府机构</c:when>
	</c:choose>&gt;<span class="current">${service.enterprise.name }</span></div>
	<div class="banner">${service.enterprise.name }</div>
	<ul class="company-nav">
		<li class="active"><a href="enter/queryEnter?eid=${service.enterprise.id }" >首页</a></li>
		<li><a href="enter/orgservices?eid=${service.enterprise.id }" >服务列表</a></li>
		<li><a href="enter/detailEnter?eid=${service.enterprise.id }" >公司信息</a></li>
	</ul>
	<div class="detail-container">
		<div class="column-l">
      	<div class="thumb">
      		<c:if test="${service.picture.contains('http')}">
      			<img class="detailimg" src="${service.picture}" width="232" height="190" title="${service.serviceName }" onerror="nofind();" />
      		</c:if>
      		<c:if test="${!service.picture.contains('http')}">
      			<img class="detailimg" src="upload/${service.picture}" width="232" height="190" title="${service.serviceName }" onerror="nofind();" />
      		</c:if>     	 	
      	</div> 
     	<ul class="choose-btn">
	    	<c:choose>
